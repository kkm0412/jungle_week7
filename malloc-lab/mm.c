...
void mm_free(void *bp)
{
    ...
    size = GET_SIZE(HDRP(bp));   //현 블록 사이즈 구하고
...
void *mm_malloc(size_t size)
{
    ...
    if(size + DSIZE <= MINBLOCKSIZE)    //free block이 pred/succ 포인터를 담을 최소 크기
        asize = MINBLOCKSIZE;
    else
